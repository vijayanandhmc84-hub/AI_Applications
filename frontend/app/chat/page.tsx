                      ...
                      {msg.sources.map((source, sidx) => (
                        <div key={sidx} className="border-t border-gray-300 pt-3">
                          {source.type === 'image' && (
                            <div className="bg-white p-3 rounded">
                              <p className="text-xs font-semibold text-gray-700 mb-2">
                                📷 {source.caption || 'Image'} {source.page ? `(Page ${source.page})` : ''}
                              </p>
                              <img
                                ...
                                onError={(e) => {
                                  ...
                                  errorDiv.textContent = `Failed to load image: ${source.url}`;
                          ...
                          {source.type === 'table' && (
                            <div className="bg-white p-3 rounded">
                              <p className="text-xs font-semibold text-gray-700 mb-2">
                                📊 {source.caption || 'Table'} {source.page ? `(Page ${source.page})` : ''}
                              </p>
                              <img
                                src={`http://localhost:8000${source.url}`}
                                ...
                                className="max-w-full rounded border border-gray-200"
                                onError={(e) => {
                                  console.error('Table image failed to load:', source.url);
                                  e.currentTarget.style.display = 'none';
                                  const errorDiv = document.createElement('div');
                                  errorDiv.className = 'text-red-500 text-xs';
                                  errorDiv.textContent = `Failed to load table: ${source.url}`;
                                  e.currentTarget.parentElement?.appendChild(errorDiv);
                                }}
                              />
                            </div>
